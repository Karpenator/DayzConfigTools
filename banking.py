...
def convert_params():
    ...
    if not os.path.exists(ultima_path):
        info_label.config(text='Ошибка: Укажите путь к папке Ultima BANK', fg='red')
        return

    if not os.path.exists(save_folder):
        os.makedirs(save_folder)

    files_list = [f for f in os.listdir(ultima_path) if f.endswith('.txt')]
    ...
    for index, file_name in enumerate(files_list):
        with open(os.path.join(ultima_path, file_name), 'r') as file:
            data = file.read()
            param_value = data.split(':')[-1].strip()
            param_name = os.path.splitext(file_name)[0].replace('BANKING_', '')
            file_path = os.path.join(save_folder, f'{param_name}.json')

        with open(file_path, 'w') as file:
            json.dump({
                'm_Steam64ID': param_name,
                'm_PlayerName': 'Survivor',
                'm_OwnedCurrency': int(param_value),
                'm_PayCheckBonus': 0,
                'm_BonusCurrency': 0,
                'm_ClanID': 'NONE'
            }, file, indent=4)
...
root = tk.Tk()
root.title('Конвертер параметров Ultima BANK')

# Создаем виджеты и располагаем их на форме
ultima_label = tk.Label(root, text='Путь к Ultima BANK')
